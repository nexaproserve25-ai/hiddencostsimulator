...
/** The complete set of levels `levelFor` can produce. Anything else in a shared link is attacker-supplied and is discarded rather than passed through. */
const validLevels = new Set(['light', 'moderate', 'heavy', 'low', 'high', 'extreme']);
...
export function readChallenge(value: string | null): ChallengePayload | null {
  ...
  try {
    ...
    return { v: 1, title: item.title, score: Math.round(item.score), level: typeof item.level === 'string' && validLevels.has(item.level) ? item.level : 'moderate', lang: item.lang as 'en' | 'ar' };
  } catch { return null; }
}
